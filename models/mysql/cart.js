...
class Cart {
  static filePath = Utility.getPath('databases/cart.json');
  ...
  static find() {
    //console.log(path.join(process.cwd(), 'databases/cart.json') );
    fs.open(this.filePath, 'r+', (err, fileDescriptor) => {
      if (err) {
        this.data = { products: [], totalPrice: 0 };
        this.save(this.data);
      } else {
        fs.readFile(this.filePath, (err, foundData) => {
          if (!err) this.data = JSON.parse(foundData);
        });
      }
    });
    return this.data;
